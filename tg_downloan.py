...
def get_list_show_tg_channel_username(id_user): #Создает словарь id_channel:channel_username
    list_user_channel = get_list_user_channel(id_user)
    list_ch = []
    db_channel = Channel.query.group_by(Channel.tg_channel_id).all()
    print(len(db_channel))
    for channel in db_channel:
        # print(channel.tg_channel_id, channel.tg_channel_username)
        if channel.id in list_user_channel:
            list_ch.append(channel.tg_channel_username)
    return list_ch
